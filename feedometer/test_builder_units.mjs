...
async function runTests() {
  ...
  // Verify it is inside WORKSPACES section
  const workspacesSectionMatch = navbarJs.match(/sectionHeader:\s*'WORKSPACES'[\s\S]*?children:\s*\[([\s\S]*?)\]/);
  assert(workspacesSectionMatch && workspacesSectionMatch[1].includes("'rss-builder'"), "RSS Builder is placed inside the WORKSPACES section");
  ...
  assert(builderHtml.includes('btn-open-in-viewer'), "Contains Open in Reader button");
  assert(builderHtml.includes('feedo-nav-drawer'), "Contains PeopleSoft Navigation Drawer");
