...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      // 4. ROUTE: /api/catalog, /api/publishers (Dynamic Publisher & Feed Directory)
      if (pathname === '/api/catalog' || pathname === '/api/publishers') {
        return await handleCatalog(request, url, env, ctx);
      }
 ...
 * Route: /api/view?url=...
 * 
 * Multi-Tiered Server-Side Feed Pipeline:
 * 1. Fast Cache Lookup:
 *    a. Cloudflare Edge Cache (caches.default) -> <10ms local datacenter hit.
 *    b. Cloudflare KV (FEEDS_KV) -> <30ms global multi-region hit.
 * 2. Feed Fetch & Multi-Tier Resilient Fallback:
 *    - Tier 1: FeedometerBot 1.0 Primary Fetch
 *    - Tier 2: Browser Profile + HTTPS Upgrade + Slash Recovery
 *    - Tier 2.5: HTML RSS Autodiscovery Engine
 *    - Tier 3: Resilient Fallback Engine (feed2json / Semantic Reader Proxy)
 * 3. XML Parsing & Image Extraction (Layer A):
 *    - Extracts <media:content>, <media:thumbnail>, <enclosure>, and inline <img>.
 * 4. Normalization, URL Canonicalization & Deduplication.
 * 5. Instant Response + Asynchronous Follow-Up (ctx.waitUntil):
 *    - Enriches missing hero images server-side.
 *    - Stores complete payload in Edge Cache and FEEDS_KV.
 *    - Asynchronously catalogs new feeds into D1 (feedometer-db).
 */
async function handleViewFeed(request, url, ctx, env) {
  ...
  const kvKey = `feed:${canonical}`;

  // STEP 1: Fast Cache Lookup (Edge Cache API + KV Storage)
  if (!noCache) {
    // 1a. Check Edge Cache (caches.default) for sub-millisecond local response
    ...
    // 1b. Check KV Cache (FEEDS_KV) for global multi-region persistence
    if (env && env.FEEDS_KV) {
      try {
        const kvCached = await env.FEEDS_KV.get(kvKey, { type: 'json' });
        if (kvCached && kvCached.ok && Array.isArray(kvCached.items)) {
          const isPop = isPopularFeed(canonical);
          const ttl = isPop ? POPULAR_FEED_CACHE_TTL : ORGANIC_FEED_CACHE_TTL;
          const resp = jsonResponse(kvCached, 200, ttl);
          resp.headers.set('X-Feedometer-Cache', 'HIT-KV');
          resp.headers.set('X-Feedometer-Fetched', kvCached.meta?.lastFetched || new Date().toISOString());
          if (isPop) resp.headers.set('X-Feedometer-Popular', 'true');

          // Warm Edge Cache asynchronously for even faster subsequent local hits
        ...
        console.warn('[Feedometer] FEEDS_KV lookup failed:', eKv && eKv.message);
      }
    }
  }
  ...
  if (!looksLikeFeedXml(rawXml)) {
    // Step A: Dedicated RSS-to-JSON Syndication Proxy (feed2json) with strict 3.5s timeout
    try {
    ...
    // Step B: Semantic Reader Proxy (Jina) with strict 3.5s timeout
    if (!parsedFromTier3) {
      try {
  ...
  if (ctx && ctx.waitUntil) {
    // 1. Asynchronous Edge Cache & OpenGraph Enrichment & KV Persistence (Non-blocking)
    ctx.waitUntil((async () => {
      try {
        // Immediate cache put for current response
        await cache.put(cacheKey, finalResponse.clone());

        // Perform server-side OpenGraph hero image extraction for articles missing images (with KV caching)
        const forOg = items.map((item) => Object.assign({}, item));
        await enrichItemsMissingHeroImages(forOg, 16, env);

        // Build fully enriched payload
        const richerPayload = Object.assign({}, payload, { items: forOg });
        const richer = jsonResponse(richerPayload, 200, cacheTtl);
        ...
        // Persist to KV for global fast access across all Cloudflare Edge nodes
        if (env && env.FEEDS_KV) {
          try {
            await env.FEEDS_KV.put(kvKey, JSON.stringify(richerPayload), {
              expirationTtl: cacheTtl
            });
          } catch (eKvPut) {
            console.warn('[Feedometer] FEEDS_KV put failed:', eKvPut && eKvPut.message);
          }
        }
      } catch (e) {
        console.warn('[Feedometer] view cache/og follow-up failed:', e && e.message);
      }
    })());
 ...
 * Concurrent Worker Pool: warms 25+ feeds in parallel chunks (5 workers)
 * Called by the scheduled handler every 6 hours, and optionally via /api/warm-cache.
 */
async function warmPopularFeeds(workerOrigin, feedUrls = POPULAR_FEEDS, concurrency = 5) {
  const results = [];
  ...
  async function worker() {
    while (activeIndex < queue.length) {
      ...
      try {
        ...
        console.log(`[Feedometer] Concurrent Warm-cache ${status}: ${feedUrl}`);
      } catch (e) {
...
async function fetchHeroImageForPageWithKv(pageUrl, env) {
  try {
    ...
    // 1. Fast KV Cache Check (<30ms, saves external subrequests)
    if (env && env.FEEDS_KV) {
      try {
        const cachedImg = await env.FEEDS_KV.get(kvKey);
        if (cachedImg !== null) {
          return cachedImg; // Empty string means previously scraped and verified no image
        }
      } catch (eKv) {}
    }
    ...
    // 3. Persist result to KV with 7-day TTL (604,800 seconds)
    if (env && env.FEEDS_KV) {
      try {
        await env.FEEDS_KV.put(kvKey, liveImg || '', {
...
async function enrichItemsMissingHeroImages(items, maxItems, env) {
  ...
  async function run() {
    while (idx < need.length) {
      ...
      const image = await fetchHeroImageForPageWithKv(item.link, env);
...
/* ------------------------------------------------------------------ */
/*  Catalog Engine: GET /api/catalog, /api/publishers                 */
/*  Delivers the 25-outlet directory and publisher sub-topic feeds.   */
/*  Multi-Tier Hierarchy: Edge Cache -> FEEDS_KV -> D1 Database       */
/* ------------------------------------------------------------------ */

/**
 * Lightweight emergency fallback catalog containing core outlets.
 * Primary source of truth is dynamic Cloudflare D1 (publishers table) + KV (catalog:publishers).
 */
...
/**
 * Handles GET /api/catalog and GET /api/publishers
 * Retrieves the publisher directory with multi-tier caching (Edge -> KV -> D1 Database).
 */
async function handleCatalog(request, url, env, ctx) {
  const cache = caches.default;
  const cacheKey = new Request(`${url.origin}/api/catalog`, { method: 'GET' });
  const noCache = url.searchParams.get('nocache') === '1';

  // 1. Edge Cache Check (<10ms)
  if (!noCache) {
    const cachedResponse = await cache.match(cacheKey);
    if (cachedResponse) {
      const resp = new Response(cachedResponse.body, cachedResponse);
  ...
  // 2. Cloudflare KV Check (<30ms)
  if (!noCache && env && env.FEEDS_KV) {
    try {
      const kvData = await env.FEEDS_KV.get('catalog:publishers', { type: 'json' });
      if (kvData && kvData.ok && kvData.publishers) {
        const resp = jsonResponse(kvData, 200, 86400); // 24-hour Edge Cache
        resp.headers.set('X-Feedometer-Cache', 'HIT-KV');
        if (ctx && ctx.waitUntil) {
          ctx.waitUntil(cache.put(cacheKey, resp.clone()));
        }
      ...
      console.warn('[Feedometer] KV catalog lookup failed:', eKv && eKv.message);
    }
  }

  // 3. Dynamic D1 Database Query (if available)
  let catalogPayload = DEFAULT_CATALOG;
  if (env && env.DB) {
    try {
      const { results: pubRows } = await env.DB.prepare(
        'SELECT domain, name, category, bg_color, description, is_popular, popularity_rank FROM publishers ORDER BY popularity_rank ASC, name ASC'
      ).all();
      ...
      if (pubRows && pubRows.length > 0) {
        ...
        for (const pub of pubRows) {
          // Find first feed for popular feeds list
          const defaultFeed = DEFAULT_CATALOG.popularFeeds.find(f => f.domain === pub.domain) || {
            name: pub.name,
            domain: pub.domain,
            url: `https://${pub.domain}`,
            category: pub.category,
            bg: pub.bg_color || '#111827'
          };
          ...
          // Use fallback sub-topic feeds or default catalog
          const existingPub = DEFAULT_CATALOG.publishers[pub.domain];
          publishers[pub.domain] = {
            name: pub.name,
            domain: pub.domain,
            bg: pub.bg_color || '#111827',
            category: pub.category,
            description: pub.description || (existingPub && existingPub.description) || '',
            feeds: (existingPub && existingPub.feeds) || [
              { title: `${pub.name} Main Feed`, url: defaultFeed.url, tag: 'Headlines', desc: pub.description || '' }
            ]
          };
        }
        ...
        catalogPayload = {
          ok: true,
          schemaVersion: 1,
          sampleFeeds: DEFAULT_CATALOG.sampleFeeds,
          popularFeeds: popularFeeds.length > 0 ? popularFeeds : DEFAULT_CATALOG.popularFeeds,
          publishers
        };
      }
    } catch (eDb) {
      console.warn('[Feedometer] D1 catalog query fallback to default:', eDb && eDb.message);
    }
  }

  // 4. Save to KV and Edge Cache
  const finalResponse = jsonResponse(catalogPayload, 200, 86400); // 24 hours
  finalResponse.headers.set('X-Feedometer-Cache', 'MISS');
  ...
  if (ctx && ctx.waitUntil) {
    ctx.waitUntil((async () => {
      try {
        await cache.put(cacheKey, finalResponse.clone());
        if (env && env.FEEDS_KV) {
          await env.FEEDS_KV.put('catalog:publishers', JSON.stringify(catalogPayload), {
            expirationTtl: 86400
          });
        }
      } catch (eCache) {
        console.warn('[Feedometer] Failed saving catalog to cache/KV:', eCache && eCache.message);
      }
    })());
  }
