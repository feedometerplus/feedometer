...
export async function findFeedsUnified({
  ...
  // 2. Boolean Query Filtering & Precision Relevance Scoring
  ...
  aggregated = aggregated.filter(f => {
    ...
    // Exact Boolean Parser evaluation (supports AND, OR, NOT, grouping, phrases)
    if (BooleanParser && typeof BooleanParser.matches === 'function') {
      const matchesBool = BooleanParser.matches(rawQ, corpus);
      if (!matchesBool) return false;
    }

    // Must match at least one meaningful search term
  ...
  aggregated = aggregated.map(f => {
    ...
    const finalRelevance = Math.min(1.0, baseRelevance + titleMatchBonus);
